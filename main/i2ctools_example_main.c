...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Setting output voltage to 5V on channel 0");
    esp_err_t ret = gp8413_set_output_voltage(dac, 5000, 0);
    if (ret != ESP_OK)
    {
    ...
    ret = gp8413_set_output_voltage(dac, 10000, 1);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to set output voltage: %s", esp_err_to_name(ret));
        gp8413_deinit(&dac);
    ...
    ESP_LOGI(TAG, "Setting output voltage to 10V on channel 1");
